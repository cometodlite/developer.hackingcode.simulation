...
(function(){
  ...
  setTimeout(kick, 650);
    ...
    const CURRENT_VERSION = 'v1.6.11(b)';
    ...
    const updateLogs = [
      ...
      },
      {
        ...
        lines: [
          ...
          '전반적인 Mobile Fix 마무리와 함께 현재 웹 배포 기준 버전을 v1.6.11(b)로 정리했습니다.'
        ]
      }
    ...
    function init() {
      ...
      syncSettingsUI();
      updateStatsUI();
      log('HCSiG 초기화 완료. (v1.6.11(b) 클릭 수정 적용)', 'system');
...
// === MOBILE SCAN OVERLAY GLOBAL FIX v1.6.11(b) ===
(function(){
  const isMobile = window.matchMedia('(max-width: 900px), (hover: none) and (pointer: coarse)').matches;
  if(!isMobile) return;
