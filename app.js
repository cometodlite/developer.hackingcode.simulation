...
/* ===== extracted script block #1 from index.html ===== */
...
/* ===== extracted script block #2 from index.html ===== */
...
/* ===== extracted script block #3 from index.html ===== */

const CURRENT_VERSION = 'v1.6.7.2';
    ...
    const updateLogs = [
      
      {
        version: 'v1.6.7.2',
        lines: [
          'GENERAL 미션을 다수 추가했습니다.',
          '업적을 다수 추가했습니다.',
          '레벨업 성장 속도를 더 완화했습니다. (필요 EXP 곡선/획득 EXP 조정)',
          '같은 아이템을 하루 안에 3회 이상 중복 구매하면 해당 아이템 가격이 x2로 적용됩니다.'
        ]
      },
{
    ...
    const missionDefs = {
      ...
      general: [
        ...
        { id: 'gen_scan_500',       name: '초광역 분석',         type: 'scans',             target: 500,   rewardCredits: 900,  desc: '누적 코드 스캔 500회' },
        { id: 'gen_hack_200',       name: '지속 침입자',         type: 'hackSuccess',       target: 200,   rewardCredits: 1200, desc: '누적 해킹 성공 200회' },

        { id: 'gen_energy_spent_2500',name:'에너지 파쇄',        type: 'energySpentTotal',  target: 2500,  rewardCredits: 1000, desc: '누적 에너지 2500 소모' },

        { id: 'gen_level_25',       name: '정예 운영자',         type: 'level',             target: 25,    rewardCredits: 900,  desc: '플레이어 레벨 25 달성' },
        { id: 'gen_level_30',       name: '최상위 운영자',       type: 'level',             target: 30,    rewardCredits: 1500, desc: '플레이어 레벨 30 달성' },

        { id: 'gen_cpu_7',          name: 'CPU 튜너 III',        type: 'cpuTier',           target: 7,     rewardCredits: 800,  desc: 'CPU 티어 7 달성' },

        { id: 'gen_energymax_35',   name: '배터리 확장 I',       type: 'energyMax',         target: 35,    rewardCredits: 450,  desc: '최대 에너지 35 달성' },
        { id: 'gen_energymax_45',   name: '배터리 확장 II',      type: 'energyMax',         target: 45,    rewardCredits: 900,  desc: '최대 에너지 45 달성' },

        { id: 'gen_shop_50',        name: '거래 중독',           type: 'shopPurchases',     target: 50,    rewardCredits: 800,  desc: '상점에서 누적 50회 구매' },

        { id: 'gen_credits_50000',  name: '데이터 자본가 III',    type: 'creditsEarnedTotal',target: 50000, rewardCredits: 1300, desc: '누적 획득 크레딧 50000 달성' },

        { id: 'gen_achieve_20',     name: '기록 수집가 IV',      type: 'achievements',      target: 20,    rewardCredits: 800,  desc: '업적 20개 달성' },

        { id: 'gen_mission_60',     name: '퀘스트 중독자',       type: 'missionsCompleted', target: 60,    rewardCredits: 1200, desc: '누적 퀘스트 60개 완료' },
    ...
    const achievementDefs = [
      ...
      ,
      // v1.6.7.2 추가 업적
      { id: 'scan_100',             name: '스캐너 마스터',      desc: '코드 스캔을 100회 수행했습니다.',             difficulty: 'normal', hidden: false },
      { id: 'hack_100_success',     name: '침입 전문가',        desc: '서버 해킹에 100회 이상 성공했습니다.',        difficulty: 'normal', hidden: false },
      { id: 'cpu_tier_3',           name: '튜너 입문',          desc: 'CPU 티어를 3 이상으로 업그레이드했습니다.',   difficulty: 'easy',   hidden: false },
      { id: 'cpu_tier_7',           name: '초과가속',           desc: 'CPU 티어를 7 이상으로 업그레이드했습니다.',   difficulty: 'hard',   hidden: true  },
      { id: 'shop_10',              name: '단골 손님',          desc: '상점에서 누적 10회 구매했습니다.',            difficulty: 'easy',   hidden: false },
      { id: 'shop_50',              name: '대형 구매자',        desc: '상점에서 누적 50회 구매했습니다.',            difficulty: 'hard',   hidden: true  },
      { id: 'energy_pack_use_5',    name: '비상 보급 I',        desc: '에너지 팩을 5회 사용했습니다.',               difficulty: 'easy',   hidden: false },
      { id: 'energy_pack_use_20',   name: '비상 보급 II',       desc: '에너지 팩을 20회 사용했습니다.',              difficulty: 'normal', hidden: true  }
    ...
    function requiredExp(level) {
      // v1.6.7.2: 성장 완화 (필요 EXP 곡선 강화)
      const lv = Math.max(1, Math.floor(level || 1));
      const linear = 30 + (lv - 1) * 15;
      const curve = Math.floor(((lv - 1) * (lv - 1)) * 0.5);
      return linear + curve;
    }
    ...
    function loadShopMeta() {
      try {
        ...
        if (!meta.purchases) meta.purchases = {};
        return meta;
      } catch (e) {
        return { date: getLocalDateKey(), daily: {}, once: {}, purchases: {} };
      }
    }
    ...
    function ensureDailyShopReset() {
      ...
      if (meta.date !== today) {
        ...
        meta.purchases = {}; // reset daily repeat-purchase counts
        saveShopMeta(meta);
    ...
    // v1.6.7.2: 같은 아이템 일일 3회 이상 중복 구매 시 가격 x2
    function getRepeatPurchaseCount(itemId) {
      const meta = ensureDailyShopReset();
      return meta.purchases[itemId] || 0;
    }

    function getEffectiveItemCost(item) {
      const base = Number(item.cost || 0);

      // 오늘 동일 아이템을 몇 번 샀는지(이미 구매한 횟수)
      const used = getRepeatPurchaseCount(item.id);

      // 구매 n회차 가격 배수 규칙:
      // 1회차 x1, 2회차 x2, 3회차 x3, 4회차 x4 ... (계속 증가)
      const nextCount = used + 1;
      const multiplier = nextCount <= 1 ? 1 : nextCount;

      return { cost: base * multiplier, multiplier, used, nextCount };
    }

    function markRepeatPurchase(itemId) {
      const meta = ensureDailyShopReset();
      meta.purchases[itemId] = (meta.purchases[itemId] || 0) + 1;
      saveShopMeta(meta);
    }
    ...
    function renderShop() {
      ...
      items.forEach(item => {
        ...
        const eff = getEffectiveItemCost(item);
        costSpan.textContent = `💰 ${eff.cost}`;
        if (eff.multiplier > 1) {
          const mult = document.createElement('span');
          mult.className = 'shop-limit-badge';
          mult.textContent = `x${eff.multiplier}`;
          mult.style.marginLeft = '6px';
          mult.style.background = '#3b1d1d';
          mult.style.border = '1px solid #5b2a2a';
          mult.style.opacity = '0.95';
          costSpan.appendChild(mult);
        }
        ...
        btn.addEventListener('click', () => {
          ...
          const effBuy = getEffectiveItemCost(item);

          if (state.credits < effBuy.cost) {
            log(`[상점] 크레딧이 부족합니다. (필요: ${effBuy.cost})`, 'shop');
...
💰 ${effBuy.cost} 크레딧이 소모됩니다.`);
            if (!ok) return;
          }
          state.credits -= effBuy.cost;
          item.buy?.();
          // mark cap usage
          markShopPurchase(item.id);
          markRepeatPurchase(item.id);

          state.stats.shopPurchaseCount++;
          log(`[상점] ${item.name} 구매 (💰 -${effBuy.cost})`, 'shop');
    ...
    function checkAchievements(reason) {
      ...
      if (state.stats.scanCount >= 100) unlockAchievement('scan_100');

      if (state.stats.hackSuccessCount >= 100) unlockAchievement('hack_100_success');

      if (state.cpuTier >= 3) unlockAchievement('cpu_tier_3');
      if (state.cpuTier >= 7) unlockAchievement('cpu_tier_7');

      if (state.stats.shopPurchaseCount >= 10) unlockAchievement('shop_10');
      if (state.stats.shopPurchaseCount >= 50) unlockAchievement('shop_50');

      const epUsed = state.stats.energyPacksUsed || 0;
      if (epUsed >= 5) unlockAchievement('energy_pack_use_5');
      if (epUsed >= 20) unlockAchievement('energy_pack_use_20');
    ...
    function setActiveTab(tabName) {
      ...
      try {
        ...
        console.warn('[MoreModal] render on tab failed:', e);
      }
    }
    ...
    btnUpdateNext.addEventListener('click', () => {
      ...
      try {
        ...
        renderUpdateLog();
        // 미션/업적 탭은 열자마자 목록 생성
        if (defaultTab === 'mission') { try { renderMissions(); } catch(e){} }
        if (defaultTab === 'achievement') { try { renderAchievements(); } catch(e){} }
    ...
    function init() {
      ...
      syncSettingsUI();
      updateStatsUI();
      log('HCSiG 초기화 완료. (v1.6.7.2)', 'system');
...
/* ===== extracted script block #4 from index.html ===== */
...
/* ===== extracted script block #5 from index.html ===== */
...
(function(){
  ...
  // NOTE: This build uses the newer 5-tab "MOBILE VIEWS" system (Status/Action/Codes/Shop/Log).
  // If those view containers exist, do NOT mount the legacy 3-tab panel switcher.
  if (document.getElementById('mobileViewStatus')) return;
...
/* ===== extracted script block #6 from index.html ===== */
...
/* ===== extracted script block #7 from index.html ===== */
...
/* ===== extracted script block #8 from index.html ===== */
...
/* ===== extracted script block #9 from index.html ===== */
...
/* ===== extracted script block #10 from index.html ===== */
